...
function EventsPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden pb-24" style={{ background: "var(--gradient-sky)" }}>
      <SkyBackdrop />
      ...
      <header className="relative px-4 pb-32 pt-10 sm:px-6 sm:pt-16">
        ...
        <div className="absolute right-[10%] top-20 hidden lg:block">
          <div className="relative animate-flight">
            <div className="relative animate-shake">
              <div className="absolute -inset-6 rounded-full blur-2xl opacity-60" style={{ background: "radial-gradient(circle, var(--brand-yellow), transparent 65%)" }} />
              <img src={logoUrl} alt="" className="relative size-32 rotate-12 object-contain drop-shadow-[0_8px_20px_rgba(255,140,0,0.45)]" />
        ...
        <div className="relative z-10 mx-auto max-w-5xl text-center">
          ...
          <p className="mx-auto mt-8 max-w-2xl font-body text-base leading-relaxed text-white/85 drop-shadow sm:mt-10 sm:text-lg">
            Connecting the QCU MSC community through workshops, hackathons, and stellar opportunities — every Saturday, defying the odds.
          </p>
        </div>
      </header>
      ...
      <section className="relative z-20 mx-auto -mt-20 mb-20 max-w-5xl px-4 sm:px-6">
        <div
          className="glass-strong flex flex-col items-stretch gap-3 rounded-[3rem] p-3 md:flex-row md:items-center"
        >
                  ...
                  style={
                    active
                      ? {
                          ...
                          color: "white",
                          boxShadow: "0 12px 24px -10px color-mix(in oklab, var(--brand-blue-deep) 55%, transparent)",
                        }
                      : {
                          background: "white",
      ...
      <main className="relative mx-auto max-w-6xl px-4 sm:px-6">
        {filtered.length === 0 ? (
          <EmptyState onReset={() => { setFilter("all"); setQuery(""); }} />
        ) : (
          <>
            {upcoming.length > 0 && (
              <>
  ...
  return (
    <header className="relative z-30 mx-auto grid max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-5 sm:px-6 sm:py-6">
      <Link to="/" className="flex min-w-0 items-center gap-2.5">
        <img src={logoUrl} alt="QCU MSC logo" className="size-9 shrink-0 object-contain sm:size-10" />
        <div className="min-w-0 leading-tight">
          ...
          <div className="hidden truncate text-[10px] uppercase tracking-[0.18em] text-white/85 drop-shadow sm:block">Microsoft Student Community</div>
        </div>
      </Link>
      <Link
        to="/"
...
function SectionHeading({ title, accent }: { title: string; accent: string }) {
  return (
    <div className="mb-12 flex items-center gap-4">
      <h2 className="font-display text-4xl font-extrabold tracking-tight text-white drop-shadow">{title}</h2>
  ...
  return (
    <article className={`group relative ${className}`}>
      <div
        ...
        style={{ boxShadow: "0 30px 60px -25px color-mix(in oklab, var(--brand-blue) 35%, transparent)" }}
      >
        ...
        <div className="p-10 sm:p-12">
          <span className="font-heading text-xs font-extrabold uppercase tracking-[0.2em]" style={{ color: event.accent }}>
            {event.tag}
          </span>
  ...
  return (
    <article className={`group ${className}`}>
      <div
        ...
        style={{ boxShadow: "0 20px 40px -20px color-mix(in oklab, var(--brand-blue) 25%, transparent)" }}
      >
        ...
        <div className="mb-3 flex items-center justify-between">
          <span className="font-heading text-[10px] font-extrabold uppercase tracking-[0.15em]" style={{ color: event.accent }}>
            {event.tag}
          </span>
        ...
        <h4 className="mb-3 font-display text-2xl font-bold tracking-tight text-brand-blue-deep">{event.title}</h4>
        <p className="mb-7 line-clamp-3 font-body text-sm leading-relaxed text-brand-blue-deep/60">{event.blurb}</p>
        <Link
          to="/events/$eventId/register"
          params={{ eventId: event.id }}
        ...
        <div key={e.id} className="group cursor-pointer text-center">
          ...
          >
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${e.accent}, color-mix(in oklab, ${e.accent} 50%, white))` }}
            >
              {e.title.split(/\s+/).map((w) => w[0]).slice(0, 2).join("").toUpperCase()}
            </div>
          </div>
          <p className="font-heading text-[10px] font-extrabold uppercase tracking-[0.2em] text-brand-blue-deep/40">{e.date}</p>
...
function EmptyState({ onReset }: { onReset: () => void }) {
  return (
    <div className="mt-12 rounded-[3rem] glass-strong p-12 text-center">
      <div className="mx-auto grid size-16 place-items-center rounded-2xl" style={{ background: "var(--gradient-cta)" }}>
        <Rocket className="size-7 text-white" />
      </div>
      <h3 className="mt-4 font-display text-2xl font-bold text-brand-blue-deep">No events match that filter</h3>
      <p className="mt-2 font-body text-sm text-brand-blue-deep/60">Try a different category or clear your search.</p>
      <button
        onClick={onReset}
...
function CallToAction() {
  return (
    <section
      ...
      style={{
        background: "linear-gradient(135deg, color-mix(in oklab, var(--brand-orange) 85%, white), color-mix(in oklab, var(--brand-yellow) 70%, white))",
